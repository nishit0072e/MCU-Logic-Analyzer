...
void initTimerDMA() {
  ...
  // Register callbacks
  HAL_DMA_RegisterCallback(&hdma_tim2_up, HAL_DMA_XFER_CPLT_CB_ID, DMA_XferCpltCallback);
  HAL_DMA_RegisterCallback(&hdma_tim2_up, HAL_DMA_XFER_ERROR_CB_ID, DMA_XferErrorCallback);
...
void startDMACapture() {
  ...
  if (period < 11) period = 11;          // Max ~6MHz
  if (period > 65535) period = 65535;    // Min ~1.1kHz with prescaler=0
